...
export default async function handler(
    ...
    try {
        const { absentTeachers, date, day } = req.body;
        
        if (!absentTeachers || !date || !day) {
            return res.status(400).json({ error: 'Parameter yang diperlukan tiada.' });
        }
        ...
        return res.status(200).setHeader('Content-Type', 'application/json').send(jsonText);

    } catch (error) {
        console.error("Ralat dalam Vercel function:", error);
        return res.status(500).json({ error: "Perkhidmatan AI gagal menjana cadangan." });
    }
};