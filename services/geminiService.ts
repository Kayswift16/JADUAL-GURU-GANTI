...
export async function generateSubstitutes(
  absentTeachers: Omit<AbsentTeacherInfo, 'notes'>[], 
  date: string, 
  day: string
): Promise<any> {
  const teacherList = absentTeachers.map(t => `- ${t.name} (Sebab: ${t.reason})`).join('\n');
  ...
  try {
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              absentTeacher: {
                type: Type.STRING,
                description: 'Nama penuh guru yang tidak hadir.',
              },
              reason: {
                type: Type.STRING,
                description: 'Sebab guru tidak hadir.',
              },
              classCovered: {
                 type: Type.STRING,
                 description: 'Kelas, waktu dan subjek yang diganti. Format WAJIB: "[Nama Tahun], [Masa Mula]-[Masa Tamat] ([Kod Subjek])". Contoh: "Tahun 1, 0720-0820 (BM)"'
              },
              substituteTeacher: {
                type: Type.STRING,
                description: 'Nama penuh guru ganti yang dicadangkan untuk slot ini.',
              }
            },
    ...
    throw new Error("Perkhidmatan AI gagal menjana cadangan.");
  }
}